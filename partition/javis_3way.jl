...
function three_way(gif::Bool=true)
    num_frames = 800
    ...
    t4 = Object((args...; color="grey", align=:left) ->
        make_text(20, "    elseif A[j] > pivot", Point(-150, -300), color, align))
    ...
    while j <= k
        if array_values[j] < pivot
            if i != j
                ...
                act!(t1, Action(fno+45:720, change(:color, "grey")))
                act!(t2, Action(fno+45:720, change(:color, "grey")))
                act!(t3, Action(fno+45:720, change(:color, "grey")))
                fno += 45
            else
                ...
                act!(t1, Action(fno+15:720, change(:color, "grey")))
                act!(t2, Action(fno+15:720, change(:color, "grey")))
                act!(t3, Action(fno+15:720, change(:color, "grey")))
                fno += 15
            end
        ...
        elseif array_values[j] > pivot
            if j != k
                ...
                act!(t4, Action(fno+45:720, change(:color, "grey")))
                act!(t5, Action(fno+45:720, change(:color, "grey")))
                act!(t6, Action(fno+45:720, change(:color, "grey")))
                fno += 45
            else
                ...
                act!(t4, Action(fno+15:720, change(:color, "grey")))
                act!(t5, Action(fno+15:720, change(:color, "grey")))
                act!(t6, Action(fno+15:720, change(:color, "grey")))
                fno += 15
            end
            ...
            act!(t7, Action(fno+15:720, change(:color, "grey")))
            act!(t8, Action(fno+15:720, change(:color, "grey")))

            fno += 15
        end
    end
    act!(t0, Action(30:fno, change(:color, "black")))
    act!(t0, Action(fno:720, change(:color, "grey")))

    for j in 1:16
        if  j <= i - 1
            act!(rectangles[j], Action(740:num_frames-15, change(:color, "green")))
        elseif j >= k + 1
            act!(rectangles[j], Action(740:num_frames-15, change(:color, "blue")))
        else
            act!(rectangles[j], Action(740:num_frames-15, change(:color, "red")))
        end
    end

    render(
        video;
        pathname = if gif "three_way.gif" else "three_way.mp4" end,
        framerate = 30
    )
    return fno
end
