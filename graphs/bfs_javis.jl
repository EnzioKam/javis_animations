...
function bfs(gif::Bool=true)
    ...
    queue_text = Object((args...; text=print_frontier(queue)) -> 
        make_text(25, "Queue: $text", Point(0, -400), "black")
    )
    ...
    while !isempty(queue)
        ...
        if !visited[v]
            ...
            fno += 15
            act!(queue_text, Action(fno:fno+15, change(:text, print_frontier(queue))))
            fno += 15
    ...
    render(
        video;
        pathname = if gif "bfs.gif" else "bfs.mp4" end,
        framerate = 30
    )
    return fno
end
